...
- The mark is typeset: the O of Source Serif 4 at opsz 60, weight 600, with the
  slash redrawn at the page's 70deg angle and set a little apart from the O. It
  ships as outlines (src/data/mark.ts, generated by scripts/mark-outlines.py).
- Must work with JavaScript disabled and under `prefers-reduced-motion`; WCAG AA
  contrast; visible keyboard focus.
- Performance is part of the pitch: the LCP element is hero text that paints
  before any script (the h1 on desktop, the h1 or the lede on phones), never the
  canvas or an image; the hero's WebGL
